...
if ( !function_exists('dbg_error_log') ) {
  ...
  */
  function dbg_error_log() {
    ...
    # The component isn't always set, if it is then we add a bit of space.
    if ($component != '') {
      $component .= ":";
    }
    ...
    if ( count($args) >= 2 ) {
      ...
      @error_log( sprintf("%s%s: %s: %s%s", $c->sysabbr, request_id_str(),
...
if ( !function_exists('fatal') ) {
  function fatal() {
    ...
    @error_log( sprintf("%s%s: FATAL: %s: %s", $c->sysabbr, $request_id_str,
      $component, vsprintf($format, $args) ));

    @error_log( $request_id_str . "================= Stack Trace ===================" );

    $trace = array_reverse(debug_backtrace());
    array_pop($trace);
    foreach( $trace AS $k => $v ) {
      @error_log( sprintf("%s ===>  %s[%d] calls %s%s%s()",
        $request_id_str,
        $v['file'],
        $v['line'],
...
*/
  function trace_bug() {
    ...
    @error_log( $c->sysabbr . $request_id_str . ": BUG: $component:". vsprintf( $format, $args ) );
    ...
    array_pop($trace);
    foreach( $trace AS $k => $v ) {
      @error_log( $request_id_str . sprintf(" ===>  %s[%d] calls %s%s%s()",
                    $v['file'],
                    $v['line'],
                    (isset($v['class'])?$v['class']:''),
                    (isset($v['type'])?$v['type']:''),
...
if ( !function_exists("deprecated") ) {
  function deprecated( $method ) {
    ...
    if ( isset($c->dbg['ALL']) || isset($c->dbg['deprecated']) ) {
      ...
      @error_log( sprintf( '%s%s: DEPRECATED: Call to deprecated method "%s"',
        $c->sysabbr, $request_id_str, $method));
      foreach( $stack AS $k => $v ) {
        @error_log( sprintf( '%s%s:  ==>  called from line %4d of %s',
          $c->sysabbr, $request_id_str, $v['line'], $v['file']));
      }
    }
  }
}
...
if ( !function_exists("request_id") ) {
  function request_id() {
    ...
    if (isset($c->request_id))
      return $c->request_id;

    if (isset($_SERVER['UNIQUE_ID'])) {
      # Use what Apache2 has provided.
      $c->request_id = $_SERVER['UNIQUE_ID'];

    } else if (isset($_SERVER['REQUEST_ID'])) {
      # Use what the web server (nginx only?) has provided.
      $c->request_id = $_SERVER['REQUEST_ID'];

    } else if (isset($_SERVER['REQUEST_METHOD'])) {
      # Make up an ID that should do the job.
      $c->request_id = hash('crc32b', (
          $_SERVER['REMOTE_ADDR'] .
      ...
      # probably run from a CLI, don't need a request ID.
      $c->request_id = '';
    }
